...
<div style="width: 100%; margin-left: auto; margin-right: auto;">
	...
	<c:forEach items="${weathers}" var="weather" varStatus="s">
		<script>
	    		var point1${weather.station.name} = new BMap.Point(${weather.station.longitude},${weather.station.latitude});
	    		var opts1${weather.station.name} = {
	    				  position : point1${weather.station.name},    // 指定文本标注所在的地理位置
	    				  offset   : new BMap.Size(-15, -20)    //设置文本偏移量
	    				}
	    		
	    		var myIcon = new BMap.Icon("./style/img/qixiang.png", new BMap.Size(20,20));
	    		var marker${weather.station.name} = new BMap.Marker(point1${weather.station.name},{icon:myIcon}); 
	    		map121.addOverlay(marker${weather.station.name});
	    		
				var label = new BMap.Label("${weather.tem}", opts1${weather.station.name}); 
					label.setStyle({
						 fontSize : "17px",
						 color : "#1010DE",
						 height : "20px",
				...
				marker${weather.station.name}.setLabel(label);
				var eventOpts1${weather.station.name} = {
						  width : 80,     // 信息窗口宽度
						  height: 115,     // 信息窗口高度
						}
				var infoWindow2${weather.station.name} =new BMap.InfoWindow("<h4 style='margin:0 0 5px 0;padding:0.2em 0'>${weather.station.name}</h4>气温：${weather.tem}<br>相对湿度：${weather.relativeHumidity}<br>风速：${weather.windVelocity2min}</div>", eventOpts1${weather.station.name}); // 创建信息窗口对象 
				marker${weather.station.name}.addEventListener("click", function(){          
					map121.openInfoWindow(infoWindow2${weather.station.name}, point1${weather.station.name}); //开启信息窗口
				});
